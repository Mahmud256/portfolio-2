// ExperienceComponent.js

import React from 'react';

const Experience = () => {
    return (
        <div id='experience'>
            <div className='flex justify-center' data-aos="flip-up">
                <div className="bg-gray-200 border dark:bg-[#1d232a] rounded-md shadow-md w-96">
                    <h2 className="text-center text-4xl font-bold text-[#15295f] dark:text-[#416fee] py-10 px-6">My Experience</h2>
                    <p className="text-gray-700 dark:text-[#bbc2d6] mb-2 px-6 pb-10">
                        I have worked with books for over 3 years at Medisucess, a leading book company. During my time, I was actively involved in the typing work of the book industry. I take pride in contributing to the success of Medisucess and playing a role in bringing valuable content to readers in Bangladesh.</p>
                </div>
            </div>
        </div>
    );
};

export default Experience;